...
export const useChatStore = defineStore('chat', {
  ...
  actions: {
    ...
    async sendChat (chatId: string, title: string, userId: string, message: string) {
        try {
            const response = await fetch('https://dark-caldron-448714-u5.uc.r.appspot.com/smart/generate', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ key: chatId, title, user_id: userId, message })
              })
