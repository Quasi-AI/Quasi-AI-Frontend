  ...
  <NuxtLayout name="auth-wrapper">
    <div class="w-full max-w-[569px] rounded-2xl bg-white py-6 text-center text-2xl lg:shadow-md">
      ...
      <div class="mx-4 flex flex-col items-center justify-center space-y-2">
        <p 
          v-if="errorMessage" 
          class="w-full bg-red-100 border-l-4 border-red-500 text-red-700 px-4 py-3 rounded-md text-sm"
        >
          {{ errorMessage }}
        </p>
        <UButton
          class="flex w-full items-center justify-center gap-2 rounded-md bg-[#DB4437] text-white hover:bg-[#C1351D] transition-colors duration-200"
          size="md"
...
const isValidForm = computed(() => {
  return name.value && !nameError.value && email.value && isValidEmail(email.value) && password.value && !passwordError.value
})
...
const validatePassword = () => {
  ...
  if (!strongPasswordRegex.test(password.value)) {
    passwordError.value = 'Password must be at least 8 characters, include an uppercase letter, a number, and a special character.'
  } else {
    passwordError.value = ''
  }
}
...
const signUpWithGoogleAsEducator = async () => {
  try {
    ...
    try {
        ...
        const response = await axios.post("https://dark-caldron-448714-u5.uc.r.appspot.com/google/signup", userData);
